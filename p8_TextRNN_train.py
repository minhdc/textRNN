...
tf.app.flags.DEFINE_string("vocab_data_path","../data/Viet22K.txt","path of vocab data.") #train-zhihu4-only-title-all.txt===>training-data/test-zhihu4-only-title.txt--->'training-data/train-zhihu5-only-title-multilabel.txt'
...
tf.app.flags.DEFINE_string("word2vec_model_path","../data/zhihu-word2vec-title-desc.bin-100","word2vec's vocabulary and vectors")
tf.app.flags.DEFINE_string("vocab_label","../data/ptit-train.txt","word2vec's vocabulary label")
#tf.app.flags.DEFINE_string("word2vec_model_path","../data/Viet22K.size300.bin","word2vec's vocabulary and vectors")
...
def main(_):
    ...
    if 1==1:
        ...
        vocabulary_word2index, vocabulary_index2word = create_voabulary(simple=None,word2vec_model_path=FLAGS.word2vec_model_path,name_scope="rnn")
        #vocabulary_word2index,vocabulary_index2word = get_vocabulary(FLAGS.vocab_file_path,name_scope="rnn")
        vocab_size = len(vocabulary_word2index)
        print("rnn_model.vocab_size:",vocab_size)
        vocabulary_word2index_label,vocabulary_index2word_label = create_voabulary_label(voabulary_label=FLAGS.vocab_label,name_scope="rnn")
        train, test, _ =  load_data_multilabel_new(vocabulary_word2index, vocabulary_word2index_label,multi_label_flag=False,training_data_path=FLAGS.training_data_path) #,training_data_path=FLAGS.training_data_path
        trainX, trainY = train
        testX, testY = test
    ...
    with tf.Session(config=config) as sess:
        ...
        tf.variable_scope('Embedding',reuse=tf.AUTO_REUSE)
        #textRNN.instantiate_weights()
